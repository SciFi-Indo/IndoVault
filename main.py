...
def initialize_ui(root):
    ...
    blank_slate.setup_blank_slate_button(root)
    create_value_boxes()
    utils.HeaderCreator(root, utils.header_style).create_all_headers()
    utils.setup_grid(root)
    utils.setup_exit_buttonIV(root)
    update_prices()
    utils.adjust_label_colors(tracker)
    #root.after(10, delayed_create_net_value_box, (0,))
    delayed_create_net_value_box(0)


def fetch_prices_async():
    tracker.fetch_excluded_coin_prices(prices_dict)
    utils.initialize_tracker(root)
...
def get_price(coin, retries=7):
    ...
    try:
        ...
        if 'price' in response_data:
            ...
            if price != "0":
                brightened_color = tracker.brighten_color("#008000", factor=2.5)

                # Ensure the widget still exists before updating
                if coin in crypto_data.price_labels and crypto_data.price_labels[coin].winfo_exists():
...
def update_prices(start_idx: int = 0):
    def worker(current_idx: int):
        ...
        if current_idx >= len(crypto_data.coins):
            if not sorted_once:
                func_args = [prices_dict, tracker.load_and_update_icon,
                             utils.update_wallet_label_for_coin, update_net_value]
                tracker.sort_prices(*func_args)
        ...
        def update_label():
            ...
            if label and label.winfo_exists():
                try:
                    label.config(text="Loading...", fg="red")  # Temporary label update
                    update_price_for_coin(coin, price, current_idx)
                    utils.adjust_label_colors(tracker)  # Ensure color adjustments happen here
                except TclError:
                    print(f"Label for {coin} has been destroyed or is invalid.")

        root.after(0, update_label)

    thread = threading.Thread(target=worker, args=(start_idx,))
    crypto_data.active_threads.append(thread)
    thread.start()


def update_price_for_coin(coin, price, current_idx):
    brightened_green = tracker.brighten_color(utils.price_style["fg"], factor=2.5)
...
def update_profit_for_coin(coin, price):
    ...
    if profit >= 0:
        brightened_color = tracker.brighten_color("#008000", factor=2.0)
        crypto_data.profit_labels[coin].config(text=f"${profit:.2f}", fg=brightened_color, font=("Arial", 16, "bold"))
    else:
        brightened_color = tracker.brighten_color("#ff0000", factor=1.5)
...
def update_break_even_price_for_coin(coin):
    ...
    if current_price != 0:
        ...
        if float(current_price) >= break_even_price:
            ...
            brightened_color = tracker.brighten_color("#ff0000", factor=2.5)
            crypto_data.break_even_labels[coin].config(text=f"${break_even_price_formatted}", fg=brightened_color)
    else:
        break_even_price_formatted = f"{break_even_price:.5f}"
...
for idx, coin in enumerate(crypto_data.coins):
    tracker.load_and_update_icon(coin)
